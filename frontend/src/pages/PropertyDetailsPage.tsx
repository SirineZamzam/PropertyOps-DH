...
export default function PropertyDetailsPage() {
  ...
  const [createUnitFor, setCreateUnitFor] = useState<Building | null>(null);

  const [editingUnit, setEditingUnit] = useState<Unit | null>(null);

  const [occupancy, setOccupancy] = useState<UnitOccupancy | null>(null);

  const [intelligenceUnit, setIntelligenceUnit] = useState<Unit | null>(null);

  async function load() {
    const properties = await apiRequest<Property[]>("/properties/");

    setProperty(properties.find((item) => item.id === id) ?? null);

    const data = await apiRequest<Building[]>(`/properties/${id}/buildings`);

    const detailed = await Promise.all(
      data.map(async (building) => ({
        ...building,

        units: await apiRequest<Unit[]>(`/buildings/${building.id}/units`),
      })),
    );

    setBuildings(detailed);
  }
  ...
  async function deleteBuilding(building: Building) {
    const confirmed = await confirmAction({
      ...
      text: "A building can only be deleted after all of its units have been removed.",
    ...
    try {
      await apiRequest(`/buildings/${building.id}`, {
        method: "DELETE",
      });

      await successAlert("Building deleted");
  ...
  async function deleteUnit(unit: Unit) {
    const confirmed = await confirmAction({
      ...
      text: "Only vacant units without lease, maintenance, or expense history can be deleted.",
    ...
    try {
      ...
      await successAlert("Unit deleted");
  ...
  async function openUnit(unit: Unit) {
    ...
    try {
      const data = await apiRequest<UnitOccupancy>(
  ...
  return (
    <div className="mx-auto max-w-7xl page-enter">
      ...
      <div className="mt-6 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button
          onClick={() => setCreateBuilding(true)}
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4 bg-cyan/60 px-6 py-5 text-deep-blue dark:bg-moss dark:text-lime-soft">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setCreateUnitFor(building)}
            ...
            <div className="grid grid-cols-2 gap-4 p-6 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6">
              {building.units.map((unit) => (
                <article key={unit.id} className="group relative">
                  <button
                    type="button"
                    title="Analyze unit"
                    onClick={(event) => {
                      event.stopPropagation();
                      setIntelligenceUnit(unit);
                    }}
                    className="absolute left-2 top-2 z-10 grid size-9 place-items-center rounded-xl bg-white/85 text-celestial shadow-sm backdrop-blur transition hover:scale-105 hover:bg-white dark:bg-phthalo/90 dark:text-lime-soft"
                  >
                    <Radar size={15} strokeWidth={1.8} />
                  </button>
                  ...
                  >
                    {(() => {
                      const UnitIcon = getUnitIcon(unit.unit_type);

                      return <UnitIcon size={48} strokeWidth={1.4} />;
                    })()}
      ...
      {createBuilding && (
        <BuildingForm
          propertyId={id}
          onClose={() => setCreateBuilding(false)}
          onSaved={async () => {
      ...
      {editingUnit && (
        <UnitForm
          unit={editingUnit}
          building={
            buildings.find((item) => item.id === editingUnit.building_id)!
          }
      ...
      {occupancy && (
        <ResidentModal
          occupancy={occupancy}
          onClose={() => setOccupancy(null)}
        />
      )}

      {intelligenceUnit && (
        <Modal
          title={`Unit ${intelligenceUnit.unit_number}`}
          eyebrow="Operational intelligence"
          wide
          onClose={() => setIntelligenceUnit(null)}
        >
          <AIInsightPanel
            scope="UNIT"
            resourceId={intelligenceUnit.id}
            resourceName={`Unit ${intelligenceUnit.unit_number}`}
          />
        </Modal>
      )}
    </div>
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (building) {
        ...
        await successAlert("Building updated");
      } else {
        await apiRequest(`/properties/${propertyId}/buildings`, {
          method: "POST",
          body: JSON.stringify({
            name,
          }),
        ...
        await successAlert("Building created");
      }
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        <FormField label="Building name" error={error}>
          <input
            value={name}
            onChange={(e) => {
        ...
        <button className="w-full rounded-2xl bg-celestial py-3.5 text-sm font-semibold text-white dark:bg-moss dark:text-lime-soft">
          {building ? "Save building" : "Create building"}
        </button>
      </form>
    </Modal>
  );
...
function UnitForm({
  ...
  const [number, setNumber] = useState(unit?.unit_number ?? "");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (unit) {
        ...
        await successAlert("Unit updated");
      } else {
        await apiRequest(`/buildings/${building.id}/units`, {
          method: "POST",
          body: JSON.stringify({
            unit_number: number,
        ...
        await successAlert("Unit created");
      }
  ...
  return (
    <Modal
      title={
        unit ? `Edit Unit ${unit.unit_number}` : `Add unit to ${building.name}`
      }
      eyebrow="Units"
      onClose={onClose}
    >
      <form onSubmit={submit} className="space-y-4">
        <FormField label="Unit number" error={error}>
          <input
            value={number}
            onChange={(e) => {
...
function ResidentModal({
  ...
  return (
    <Modal title="Current resident" eyebrow="Occupied unit" onClose={onClose}>
      {!tenant ? (
        <p className="text-sm text-deep-blue/50 dark:text-white/45">
          No active tenant was found.
        </p>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center gap-4 rounded-2xl bg-cyan/50 p-4 text-deep-blue dark:bg-moss dark:text-lime-soft">
            ...
            <div>
              ...
              <p className="mt-1 text-xs opacity-55">Current tenant</p>
            </div>
          </div>

          <div className="flex items-center gap-3 rounded-2xl border border-celestial/10 p-4 dark:border-ash/10">
            <Mail size={17} className="text-celestial dark:text-ash" />

            <span className="text-sm">{tenant.email}</span>
          </div>

          <div className="flex items-center gap-3 rounded-2xl border border-celestial/10 p-4 dark:border-ash/10">
            <Phone size={17} className="text-celestial dark:text-ash" />
